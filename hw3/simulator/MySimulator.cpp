...
void HouseCell::setDirtLevel(int dirt_level) { this->dirt_level = dirt_level; }
...
std::string MySimulator::statusString() const {
  if (stepsList.size() >= 1 && stepsList[stepsList.size() - 1] == Step::Finish)
    return "FINISHED";
  if (robotDied())
    return "DEAD";
  return "WORKING";
...
bool MySimulator::createOutput(std::string output_file_path,
  ...
  try {
    ...
    if (!file.is_open()) {
      std::cerr << "Unable to open file: " << output_file_path << std::endl;
      return true;
    }
    ...
    file << "InDock = " << (isInDocking() ? "TRUE" : "FALSE") << std::endl;
    file << "Score = " << calculate_score(finished_in_time) << std::endl;
    file << "Steps:" << std::endl;
...
int MySimulator::calculate_score(bool finished_in_time) const {
  if (!finished_in_time) {
    return 2 * max_steps + 300 * initial_dirt + 2000;
  }
  std::string status = statusString();
  int dirt_left = getDirtLeft();
  size_t steps = stepsList.size();
  if (steps > 0 && stepsList[steps - 1] == Step::Finish) {
    steps--;
  }
  ...
  } else if (status == "FINISHED" && (!isInDocking())) {
    return max_steps + (dirt_left * 300) + 3000;
  }
  return steps + (dirt_left * 300) + (isInDocking() ? 0 : 1000);
}

int MySimulator::getMaxSteps() const { return max_steps; }

bool MySimulator::isError() const { return error; }
