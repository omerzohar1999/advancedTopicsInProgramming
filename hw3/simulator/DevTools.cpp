...
DevTools::DevTools() {
  try {
    ...
    std::cerr << "Exception reading from config file: " << e.what()
              << std::endl;
...
bool DevTools::isVisualizationEnabled() { return is_visualization_enabled; }
  ...
  try {
    ...
    if (std::filesystem::exists(fileName)) {
      ...
      if (!std::filesystem::remove(fileName)) {
        std::cerr << "File " << fileName << " could not be removed."
                  << std::endl;
      }
    }
